...
1. **약인공지능 (Weak AI)**
   - 특정 작업을 수행하는 데 특화된 인공지능. 현재 대부분의 AI 시스템이 여기에 해당.
   - 예시: 얼굴 인식, 음성 인식 시스템, 추천 알고리즘 등.
   
2. **강인공지능 (Strong AI)**
   - 인간과 동일한 수준의 지능을 가진 AI로, 범용적인 문제 해결이 가능한 시스템. 아직 연구 단계에 있음.
   - 예시: 자율적으로 인간처럼 사고하고, 감정을 이해하며, 상황에 맞는 결정을 내리는 AI.